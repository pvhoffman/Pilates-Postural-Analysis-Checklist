...
xxSDKVERSION = iphonesimulator8.0
...
clean:
	...
	xcodebuild -configuration Debug -sdk iphoneos8.0 -target $(xxTARGET) clean
	xcodebuild -configuration Release -sdk iphoneos8.0 -target $(xxTARGET) clean
	-rm Classes/*backup.*
	-rm Classes/*backup~
...
device:
	xcodebuild -sdk iphoneos8.0 -target $(xxTARGET) -configuration Debug 
	/usr/bin/xcrun -sdk iphoneos8.0 PackageApplication -v $(FULL_PATH_TO_DEBUG_APP) -o $(FULL_PATH_TO_DEBUG_IPA)
...
release:
	xcodebuild -sdk iphoneos8.0 -target $(xxTARGET) -configuration Release 
	/usr/bin/xcrun -sdk iphoneos8.0 PackageApplication -v $(FULL_PATH_TO_RELEASE_APP) -o $(FULL_PATH_TO_RELEASE_IPA)
